...
セットアップ完了後、この `pdf-generator-kit` フォルダは自動的に **`.pdf-generator-kit`** にリネームされます。名前が `.` から始まる隠しファイルとなりますが、中身は削除されずに残っています。
</div>
...
このサンプルは、「原稿スタイルガイド」兼「デザイン見本」兼「テンプレート」です。
...
3. **SAMPLE.pdf を開く**
   ...
   | 役割 | 説明 |
   |---|---|
   | **原稿スタイルガイド** | SAMPLE.md と照らし合わせると、Markdownの記法とPDFの見た目の対応が分かる |
   | **デザイン見本** | このキットで制作するPDF教材の、仕上がりのイメージを持てる |
...
このキットの**原稿スタイルガイドは `articles/SAMPLE.md` そのもの**です。
...
| | 用途 | コマンド | 指定するもの |
|---|---|---|---|
| **シングル変換** | 1ファイルだけ | `npm run pdf ファイルパス` | ファイルへのパス |
| **一括変換** | フォルダ内のすべて | `npm run pdf:all フォルダパス` | フォルダへのパス |

<div class="column-box">
<div class="column-title">重要</div>

シングル変換は「**ファイルのパス**」、一括変換は「**フォルダのパス**」（相対パス）を指定します。対象がファイルかフォルダかの違いだけで、どちらもパス指定なので覚えやすいです！
</div>
...
- `npm run pdf:all` の後ろに、**フォルダのパス**を指定します。
- **フォルダのパスを省略すると、デフォルトの原稿フォルダ `articles/` が自動で対象になります。**
...
Cursor、Google Antigravity、Claude Codeなどのコーディング特化型AIツールを使って制作している場合、セットアップ時に展開された `SKILL.md` をAIが自動で読み込みます。

これにより、AIがこのキット特有のルールを把握した状態でサポートしてくれるようになります。特別な指示は不要で、そのまま相談するだけで問題ありません。
